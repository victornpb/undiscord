...
(function () {
	...
	popup = window.open(
		"",
		"",
		`top=0,left=${screen.width - 800},width=850,height=${screen.height}`
	);
	...
	startBtn.onclick = async (e) => {
		...
		fileSelection.addEventListener(
			"change",
			() => {
	...
	async function deleteMessages(
		...
		const printDelayStats = () =>
			log.verb(
				`Delete delay: ${deleteDelay}ms, Search delay: ${searchDelay}ms`,
				`Last Ping: ${lastPing}ms, Average Ping: ${avgPing | 0}ms`
			);
		...
		async function recurse() {
			...
			if (resp.status === 202) {
				...
				log.warn(
					`This channel wasn't indexed, waiting ${w}ms for discord to index it...`
				);
				await wait(w);
			...
			if (!resp.ok) {
				...
				if (resp.status === 429) {
					...
					log.warn(
						`Being rate limited by the API for ${w}ms! Increasing search delay to ${searchDelay}ms, adjusting base search delay to ${baseSearchDelay}ms`
					);
					printDelayStats();
				...
				searchDelay =
					baseSearchDelay +
					(searchDelay - baseSearchDelay) * delayReductionGeometricFactor;
			...
			const messagesToDelete = discoveredMessages.filter((msg) => {
				return (
					msg.type === 0 || msg.type === 6 || (msg.pinned && includePinned)
				);
			});
			...
			const end = () => {
				log.success(
					`Ended at ${new Date().toLocaleString()}! Total time: ${msToHMS(
						Date.now() - start.getTime()
					)}`
				);
				printDelayStats();
				log.verb(
					`Rate Limited: ${throttledCount} times. Total time throttled: ${msToHMS(
						throttledTotalTime
					)}.`
				);
			...
			log.info(
				...
				`(Messages in current page: ${data.messages.length}, To be deleted: ${messagesToDelete.length}, System: ${skippedMessages.length})`,
				`offset: ${offset}`
			);
			printDelayStats();
			...
			if (messagesToDelete.length > 0) {
				if (++iterations < 1) {
					...
					if (
						!(await ask(
							`Do you want to delete ~${total} messages?\nEstimated time: ${etr}\n\n---- Preview ----\n` +
								messagesToDelete
									.map(
										(m) =>
				...
				for (let i = 0; i < messagesToDelete.length; i++) {
					...
					if (stopHndl && stopHndl() === false)
						return end(log.error("Stopped by you!"));

					log.debug(
						`${(((delCount + 1) / grandTotal) * 100).toFixed(2)}% (${
							delCount + 1
						}/${grandTotal})`,
						`Deleting ID:${redact(message.id)} <b>${redact(
					...
					if (!resp.ok) {
						...
						if (resp.status === 429) {
							...
							log.warn(
								`Being rate limited by the API for ${w}ms! Increasing delete delay to ${deleteDelay}ms, adjusting base delete delay to ${baseDeleteDelay}ms`
							);
							printDelayStats();
							log.verb(`Cooling down for ${w * 2}ms before retrying...`);
							await wait(w * 2);
						...
						deleteDelay =
							baseDeleteDelay +
							(deleteDelay - baseDeleteDelay) * delayReductionGeometricFactor;
				...
				if (skippedMessages.length > 0) {
					...
					log.verb(
						`Found ${skippedMessages.length} system messages! Decreasing grandTotal to ${grandTotal} and increasing offset to ${offset}.`
					);
				}

				log.verb(
					`Searching next messages in ${searchDelay}ms...`,
		...
		log.debug(
			`authorId="${redact(authorId)}" guildId="${redact(
				guildId
			)}" channelId="${redact(channelId)}" minId="${redact(
				minId
			)}" maxId="${redact(maxId)}" hasLink=${!!hasLink} hasFile=${!!hasFile}`
		);
