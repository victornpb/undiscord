			...
			for (let i = 0; i < messagesToDelete.length; i++) {
				...
				if (!resp.ok) {
					...
					if (resp.status === 429) {
						...
						log.warn(
							`Being rate limited by the API for ${w}ms! Adjusted delete delay to ${deleteDelay}ms.`
						);
						printDelayStats();
						log.verb(`Cooling down for ${w * 2}ms before retrying...`);
						await wait(w * 2);
...
function initUI() {
	...
	btn =
		createElm(`<div id="undicord-btn" tabindex="0" role="button" aria-label="Delete Messages" title="Delete Messages">
